...
public static val TEMPERATURA_INCORRECTA = 'Temperatura_incorrecta'
	...
	def checkGreetingStartsWithCapital(Granja granja) {
		if (!Character.isUpperCase(granja.name.charAt(0))) {
			warning('Nombre de la granja demasiado largo', 
	...
	def checkDispositvoBarrerar(Dispositivos dispositivo){
		 if(dispositivo.nombre.getName.equals("BARRERA_SEGURIDAD") 
		&& !checkBarrera(dispositivo.accion) ){
					error("La accion no es la correcta para este dispositivo",
	...
	def checkDispositivoTemperatura(Dispositivos dispositivo){
		if(!dispositivo.nombre.getName.equals("TEMPERATURA") &&
				dispositivo.temperatura != null)
				warning("Recuerde que necesita el lugar INTERIOR/EXTERIOR ",
					GratextPackage.Literals.DISPOSITIVOS__NOMBRE,
					TEMPERATURA_INCORRECTA)
			
	}
	
	def checkTemperatura(Estado estado) {
	...
	def checkCodigo(Dispositivos dispositivo){
		if(dispositivo.codigo.length> 20){
			warning("No se recomiendan nombres de codigos de mas de 20 carcateres",
				GratextPackage.Literals.DISPOSITIVOS__CODIGO, NAME_LONG
			)
		}
	...
	def checkDispositivoCamara(Dispositivos dispositivo){
		if(dispositivo.nombre.getName.equals("CAMARA") 
		&& !checkCamara(dispositivo.accion) ){
					error("La accion no es la correcta para este dispositivo",
					GratextPackage.Literals.DISPOSITIVOS__ACCION,
					ACCION_INCORRECTA)
	...
	if(accion.nombreAccion.getName.equals("ABRIR") ||
				accion.nombreAccion.getName.equals("CERRAR")|| 
				accion.nombreAccion.getName.equals("SACAR") ||
				accion.nombreAccion.getName.equals("GRABAR")|| 
				accion.nombreAccion.getName.equals("ACTIVAR")||
				accion.nombreAccion.getName.equals("DESACTIVAR"))
				true
	}
	
	@Check
	def checkDispositivoEstado(Dispositivos dispositivo){
		if((dispositivo.nombre.getName.equals("DEPOSITO_LECHE") ||
			dispositivo.nombre.getName.equals("EMERGENCIA") ||
			dispositivo.nombre.getName.equals("FUEGO_HUMO") ||
			dispositivo.nombre.getName.equals("INUNDACION") ||
			dispositivo.nombre.getName.equals("VIENTO") ||
			dispositivo.nombre.getName.equals("ROTURA_CRISTAL")||
			dispositivo.nombre.getName.equals("CO2") ||
			dispositivo.nombre.getName.equals("LLUVIA") ||
			dispositivo.nombre.getName.equals("ESTACION_METEOROLOGICA") )
		&& !checkEstado(dispositivo.accion) ){
					error("La accion no es la correcta para este dispositivo",
					GratextPackage.Literals.DISPOSITIVOS__ACCION,
					ACCION_INCORRECTA)
	...
	def checkDispositivoADE(Dispositivos dispositivo){
		if((dispositivo.nombre.getName.equals("ROBOT_LIMPIADOR") ||
			dispositivo.nombre.getName.equals("LUMINOSIDAD") ||
			dispositivo.nombre.getName.equals("NEVERAS") ||
			dispositivo.nombre.getName.equals("RADIOFRECUENCIA") )
